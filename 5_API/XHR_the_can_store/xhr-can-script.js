...
function retrieveProducts() {
  ...
  request.onload = function() {
    if (request.status !== 200) {
      console.log(
        `Request for "products.json" failed: ${request.status}: ${request.statusText}`
      );
    } else {
      products = request.response; // dont forget a variable for request.response.
      initialize();
    }
  };
  request.send();
...
function initialize() {
  ...
  function fetchBlob(product) {
    ...
    request.onload = function() {
      if (request.status !== 200) {
        console.log(
          `Request for ${product.image} faled ${request.status}: ${request.statusText}`
        );
      } else {
        let blob = request.response; // dont forget a variable for request.response.
